# Machine Learning Codes, Ideas, and Models
![image](https://github.com/user-attachments/assets/868c1825-b66c-4d9d-8859-4ccb88a9bb4c)

Welcome a concise collection of projects, notebooks, data, and competition entries documenting my applied machine learning work and learning journey.


## Projects & Competitions

- Vegetation-Mapping — competition; Silver medal. 

- WIDS-2025 — competition; score: 77. 
## Notable Projects

- Kepler-Exoplanet — my first-ever project. 

- Airbnb New User Booking Prediction — one of my early
## Repository Structure (high level)

-- Airbnb-NUB — notebooks and data for the Airbnb new user booking prediction project


-- Kepler-Exoplanet — exoplanet EDA and modeling notebooks


-- Vegetation-Mapping — competition notebooks and data 


-- WIDS-2025 — competition code, notebooks, and requirements.txt 

## Notes

- This repository is a learning log — notebooks were developed iteratively. Expect exploratory code and experiments.
- If you want me to tidy or standardize a specific project folder (convert notebooks to scripts, add reproducible run scripts, or create environment files), tell me which one and I will help.

